...
const Overview      = dynamic(() => import('./sections/Overview'),           { loading: () => <Loader/> })
const Growth        = dynamic(() => import('./sections/Growth'),             { loading: () => <Loader/> })
const Chat          = dynamic(() => import('./sections/Chat'),               { loading: () => <Loader/> })
const Prayer        = dynamic(() => import('./sections/Prayer'),             { loading: () => <Loader/> })
const Testimony     = dynamic(() => import('./sections/Testimony'),          { loading: () => <Loader/> })
const Announcements = dynamic(() => import('./sections/Announcements'),      { loading: () => <Loader/> })
const Meetings      = dynamic(() => import('./sections/Meetings'),           { loading: () => <Loader/> })
const Members       = dynamic(() => import('./sections/Members'),            { loading: () => <Loader/> })
const SoulTracker   = dynamic(() => import('./sections/SoulTracker'),        { loading: () => <Loader/> })
const MemberVerif   = dynamic(() => import('./sections/MemberVerification'), { loading: () => <Loader/> })
const Volunteer     = dynamic(() => import('./sections/Volunteer'),          { loading: () => <Loader/> })
const Attendance    = dynamic(() => import('./sections/Attendance'),         { loading: () => <Loader/> })
const Events        = dynamic(() => import('./sections/Events'),             { loading: () => <Loader/> })
const Reports       = dynamic(() => import('./sections/Reports'),            { loading: () => <Loader/> })
const Settings      = dynamic(() => import('./sections/Settings'),           { loading: () => <Loader/> })
const PastoralPulse = dynamic(() => import('./sections/PastoralPulse'),      { loading: () => <Loader/> })
const MembershipCard= dynamic(() => import('./sections/MembershipCard'),     { loading: () => <Loader/> })
const BranchDash    = dynamic(() => import('./sections/BranchDashboard'),    { loading: () => <Loader/> })

function Loader() {
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:280, color:'var(--t-3)', gap:8, fontSize:13 }}>
      <svg className="anim-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="16" height="16">
...
function BroadcastModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(10,26,17,0.75)', backdropFilter:'blur(6px)', padding:16 }} onClick={onClose}>
      <div style={{ background:'white', borderRadius:20, width:'100%', maxWidth:480, boxShadow:'0 24px 64px rgba(10,26,17,0.35)', overflow:'hidden' }} onClick={e => e.stopPropagation()}>
        <div style={{ background:'var(--grad-brand)', padding:'18px 22px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:11.5, color:'rgba(255,255,255,0.6)' }}>Send to your congregation</div>
            </div>
          </div>
        ...
        <div style={{ padding:'20px 22px' }}>
          {sent ? (
            <div style={{ textAlign:'center', padding:'2rem' }}>
              ...
              <div style={{ fontSize:13, color:'var(--t-2)' }}>Your message has been dispatched.</div>
            </div>
          ) : (
            <form onSubmit={send}>
              <div style={{ marginBottom:14 }}>
                ...
                <textarea style={{ width:'100%', padding:'10px 14px', border:'1.5px solid var(--border-md)', borderRadius:10, fontSize:13.5, background:'var(--s-1)', color:'var(--t-1)', fontFamily:'var(--font-body)', outline:'none', resize:'vertical', boxSizing:'border-box' }} rows={4} placeholder="Type your broadcast message…" value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))} required/>
              </div>
...
const NAV = [
  ...
  { section:'PEOPLE', items:[
    { key:'members',      label:'Members',             icon:'M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M12 7a4 4 0 1 0 0-8 4 4 0 0 0 0 8z' },
    ...
    { key:'volunteer',    label:'Workforce',           icon:'M9 11l3 3L22 4M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11' },
...
const PAGES: Record<string, React.ComponentType<any>> = {
  ...
  reports:Reports, settings:Settings, pastoral:PastoralPulse,
  membcard:MembershipCard, branches:BranchDash,
}

// ── Sidebar — defined OUTSIDE Dashboard so React never remounts it ────────────
...
const Sidebar = memo(function Sidebar({ page, onNav, onCloseMobile }: SidebarProps) {
  ...
  return (
    <>
      {/* Header */}
      <div style={{ padding:'16px 14px 14px', borderBottom:'1px solid rgba(255,255,255,0.07)', background:'rgba(0,0,0,0.15)', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
      ...
      <div style={{ padding:'10px', flex:1, overflowY:'auto' }}>
        {NAV.map(sec => (
          <div key={sec.section}>
            <div style={{ fontSize:8.5, color:'rgba(201,168,76,0.55)', padding:'14px 12px 5px', letterSpacing:'.12em', textTransform:'uppercase', fontWeight:700, display:'flex', alignItems:'center', gap:8 }}>
            ...
            {sec.items.map((item: any) => (
              <div
                ...
                onKeyDown={e => (e.key === 'Enter' || e.key === ' ') && handleNav(item.key)}
              >
      ...
      {/* User block */}
      <div style={{ padding:'12px 14px', borderTop:'1px solid rgba(255,255,255,0.08)', background:'rgba(0,0,0,0.15)', flexShrink:0 }}>
...
const BOTTOM_NAV = [
  ...
  { key:'members',   label:'Members',  icon:'M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0-4 4v2M12 7a4 4 0 1 0 0-8 4 4 0 0 0 0 8z' },
  { key:'prayer',    label:'Prayer',   icon:'M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z' },
...
export default function Dashboard() {
  ...
  const handleNav         = useCallback((key: string) => setPage(key), [])
    ...
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', background:'var(--s-1)' }}>
      ...
      {mobileOpen && (
        <div style={{ position:'fixed', inset:0, zIndex:50, display:'flex' }}>
          <div style={{ width:216, background:'var(--dark)', display:'flex', flexDirection:'column', overflowY:'auto' }}>
            <Sidebar page={page} onNav={handleNav} onCloseMobile={handleCloseMobile}/>
          </div>
          <div style={{ flex:1, background:'rgba(0,0,0,0.55)' }} onClick={handleCloseMobile}/>
        </div>
      )}
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
        ...
        <div className="topbar">
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            <button className="btn btn-ghost btn-icon mob-only" onClick={() => setMobileOpen(true)} aria-label="Open menu">
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <div style={{ width:38, height:38, borderRadius:12, background:'linear-gradient(135deg,var(--brand) 0%,var(--brand-md) 100%)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:17, boxShadow:'0 3px 10px rgba(27,67,50,0.25)', flexShrink:0 }}>{meta.emoji}</div>
              <div>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            <button className="btn btn-ghost btn-icon" aria-label="Notifications">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="17" height="17"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
            </button>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'20px 24px', background:'linear-gradient(160deg, #EDECEA 0%, #F0EFE8 60%, #EAE9E0 100%)' }}>
          ...
          {Page
            ? <Page onNavigate={handleNav}/>
            : (
              <div style={{ textAlign:'center', padding:'4rem 2rem', color:'var(--t-3)' }}>
        ...
        <div style={{ height:56, background:'var(--dark-2)', borderTop:'1px solid rgba(255,255,255,0.08)', display:'flex', alignItems:'center', justifyContent:'space-around', flexShrink:0, boxShadow:'0 -4px 20px rgba(13,31,22,0.25)' }}>
          {BOTTOM_NAV.map(item => (
            <button
              ...
              style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:3, padding:'6px 12px', border:'none', background:'none', cursor:'pointer', position:'relative', transition:'all .15s', minWidth:44 }}
            >
              <svg viewBox="0 0 24 24" fill={page === item.key ? 'var(--gold)' : 'none'} stroke={page === item.key ? 'var(--gold)' : 'rgba(255,255,255,0.35)'} strokeWidth="1.8" width="22" height="22">
                <path d={item.icon}/>
              </svg>
