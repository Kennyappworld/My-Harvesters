    ...
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', background:'var(--dark)', color:'white', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center', padding:'3rem 1.5rem', position:'relative' }}>
        <div style={{ width:60, height:60, background:'var(--grad-brand)', borderRadius:18, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 28px', boxShadow:'0 8px 32px rgba(27,67,50,0.45)' }}>
          {logoUrl
            ? <img src={logoUrl} alt="logo" style={{ width:60, height:60, borderRadius:18, objectFit:'cover' }}/>
        ...
        <p style={{ fontSize:'clamp(14px,2vw,17px)', color:'rgba(255,255,255,.45)', lineHeight:1.7, maxWidth:480, marginBottom:38 }}>
          Member growth. Soul tracking. Community. Prayer. Every branch, one view.
        </p>
        ...
        <div style={{ display:'flex', gap:12, flexWrap:'wrap', justifyContent:'center' }}>
          ...
          <Link href="/signup" style={{ textDecoration:'none', display:'inline-flex', alignItems:'center', padding:'13px 26px', border:'1px solid rgba(255,255,255,.18)', borderRadius:20, fontSize:14, color:'rgba(255,255,255,.7)', fontWeight:500, fontFamily:'var(--font-body)', transition:'all .15s' }}>Register as a worker</Link>
        </div>

        {/* Capability strip — "Pastoral pulse" removed (internal feature) */}
        <div style={{ marginTop:56, display:'flex', gap:0, flexWrap:'wrap', justifyContent:'center', borderTop:'1px solid rgba(255,255,255,.08)', paddingTop:36, maxWidth:640, width:'100%' }}>
          {['Growth analytics', 'Soul tracker', 'Community chat', 'Prayer wall', 'Workforce', 'Meetings', 'Digital ID cards'].map((f, i, arr) => (
            <span key={f} style={{ fontSize:12, color:'rgba(255,255,255,.32)', padding:'4px 12px', borderRight: i < arr.length-1 ? '1px solid rgba(255,255,255,.1)' : 'none', whiteSpace:'nowrap', lineHeight:2 }}>{f}</span>
          ))}
        </div>
      </div>

      {/* Footer */}
      <footer style={{ borderTop:'1px solid rgba(255,255,255,.07)', padding:'1rem 1.5rem', display:'flex', flexDirection:'column', alignItems:'center', gap:6 }}>
